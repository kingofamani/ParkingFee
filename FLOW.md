...
graph TD
    ...
    subgraph "全國一鍵查詢"
        J["使用者點擊<br>「一鍵查詢全國」按鈕"] --> K["前端 JS 發送請求到 Flask 後端<br>(/api/query-all)"];
        K --> L["後端遍歷所有縣市 API<br>並記錄個別結果"];
        L -- "完成" --> P["後端彙總所有縣市的<br>成功/失敗/無資料狀態"];
        P --> Q["後端將完整狀態列表回傳給前端"];
        Q --> R["前端正規化所有成功資料<br>並為所有狀態動態生成結果卡片"];
        R --> S["將查詢參數與結果<br>存入 Local Storage"];
    end
``` 